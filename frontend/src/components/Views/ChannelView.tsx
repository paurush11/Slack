...
const ChannelView: React.FC<ChannelViewProps> = ({ }) => {
  ...
  const channelData: GetChannelQuery | null = useSelector((state: RootState) => state.myChannelData.data)


  return (
    <Box display={"flex"} flexGrow={1} height={"100%"}>
      <Box width={"100%"} p={4}>
        {channelData && <Typography key={(channelData as GetChannelQuery)?.getChannel.Name} variant="h1">
